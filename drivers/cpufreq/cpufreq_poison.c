...
static unsigned int dt2w_tap_distance_x, dt2w_tap_distance_y, dt2w_time_for_tap;
static bool dt2w_on_buttons_enabled, pocket_mod_enabled, revert_dt2w_area_bounded_by_xy;
...
static ssize_t show_dt2w_tap_distance_x(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", dt2w_tap_distance_x);
}

static ssize_t store_dt2w_tap_distance_x(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
	if (res >= 0 && input > 0 && input <= 720) {

		dt2w_tap_distance_x = input;
        dt2w_feather_x = dt2w_tap_distance_x;
		if (!suspended)
			thunderx_update_min_max_allcpus();
	}
	else return -EINVAL;
...
static ssize_t show_dt2w_tap_distance_y(struct kobject *kobj, struct attribute *attr, char *buf)
{
	return sprintf(buf, "%u\n", dt2w_tap_distance_y);
}

static ssize_t store_dt2w_tap_distance_y(struct kobject *kobj, struct attribute *attr, const char *buf, size_t count)
{
	ssize_t res;
	unsigned long input;
	res = strict_strtoul(buf, 0, &input);
	if (res >= 0 && input > 0 && input <= 1280) {

		dt2w_tap_distance_y = input;
        dt2w_feather_y = dt2w_tap_distance_y;
		if (!suspended)
			thunderx_update_min_max_allcpus();
	}
	else return -EINVAL;
...
define_global_rw_attr(dt2w_tap_distance_x);
define_global_rw_attr(dt2w_tap_distance_y);
...
static struct attribute * thunderx_attributes[] = {
    ...
    &dt2w_tap_distance_x_attr.attr,
    &dt2w_time_for_tap_attr.attr,
    &dt2w_tap_distance_y_attr.attr,
	NULL,
};
...
// ********* Venom kernel additions for A7000 (aio_row) inits **********
    ...
    dt2w_tap_distance_y=50;
    dt2w_tap_distance_x=50;
    dt2w_time_for_tap=600;
